...
if False:
    ...
    clf = RandomForestClassifier(max_depth=7, random_state=0, n_estimators=1000,
                                 max_features=0.05, bootstrap=True)
    clf.fit(X, y)

#live_model.class_std_conf = np.maximum(live_model.class_std_conf, 5)
live_model.med_class_duration = np.ones(len(live_model.med_class_duration))*10000
...
for ds_id in sorted(list(set(dataset_id_final_test))):
    ...
    fname = '/angel_workspace/ros_bags/hmm-ds=%s.png' % dsets[ds_id].split('/')[-3]
    viz_step_finished_conf(live_model, X, true_step, time_windows, valid,
